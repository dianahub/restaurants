...
import { getNearbyRestaurants, type RestaurantNFT } from '../utils/restaurantData'
...
function localizeRestaurant(r: RestaurantNFT & { distanceKm: number }, lang: 'en' | 'es'): NearbyRestaurant {
  return {
    ...r,
    localizedName: lang === 'es' ? r.name_es || r.name_en : r.name_en || r.name_es,
    localizedDescription: lang === 'es' ? r.description_es || r.description_en : r.description_en || r.description_es,
    localizedSpecial:
      lang === 'es'
...
export function useNearbyRestaurants(
  ...
  useEffect(() => {
    setRestaurants((prev) => prev.map((r) => localizeRestaurant(r, lang)))
  }, [lang])
